...
internal sealed class EmployeeService : IEmployeeService
{
    ...
    )
    {
        ...
        var employeesWithMetaData = await _repository.EmployeeRepository.GetEmployeesAsync(
            companyId,
    ...
    {
        await CheckIfCompanyExists(companyId, trackChanges);
        var employeeFromDb = await GetEmployeeForCompanyAndCheckIfItExists(
            companyId,
            id,
            trackChanges
        );
    ...
    )
    {
        ...
        var employeeDb = await GetEmployeeForCompanyAndCheckIfItExists(
            companyId,
            id,
            empTrackChanges
        );
    ...
    {
        var company =
            await _repository.CompanyRepository.GetCompanyAsync(companyId, trackChanges)
    ...
    {
        var employeeDb =
            await _repository.EmployeeRepository.GetEmployeeAsync(companyId, id, trackChanges)
